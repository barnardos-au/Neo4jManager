...
namespace Neo4jManager.V2
{
    ...
    public class Neo4jV2ServiceInstanceProvider : Neo4jV2ProcessBasedInstanceProvider, INeo4jInstance
    {
        ...
        private async Task InstallService(CancellationToken token)
        {
            await Task.Run(() =>
            {
                using (var process = GetProcess("Install-Neo4jServer"))
                {
                    process.Start();
                    process.WaitForExit();
                }
        ...
        private async Task StartService(CancellationToken token)
        {
            await Task.Run(() =>
            {
                using (var process = GetProcess("Start-Neo4jServer"))
                {
                    process.Start();
                    process.WaitForExit();
                }
        ...
        {
            using (var process = GetProcess("Stop-Neo4jServer"))
            {
                process.Start();
                process.WaitForExit();
            }
        ...
        {
            using (var process = GetProcess("Uninstall-Neo4jServer"))
            {
                process.Start();
                process.WaitForExit();
            }
        ...
        private string GetPowerShellArguments(string command)
        {
            ...
            builder
                ...
                .Append(" -Command ")
                .Append(quotes)
                .Append($@"Import-Module '{neo4jHomeFolder}\bin\Neo4j-Management.psd1'; Exit ('{neo4jHomeFolder}' | {command})")
                .Append(quotes);
